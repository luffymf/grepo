	...
	var less={async:true,fileAsync:true};
		(function(){
			var ctx={hash:"gm_",domain:"botsoft.org"};
				var eval_ctx=ctx.eval_ctx=function(js){return(function(){	
					return eval(js);
					}).call(ctx);};
						var bot=ctx.bot={
							domain:"botsoft.org",
							ctx:ctx,version:"25.08.2018 #1",
							controls:{},
							templates:{},
							models:{},
							autoreload:{count:0},
							ajax:"//botsoft.org/pt/bot/ajaxv2/",
							active:false,
							requests:0,
							failRequests:0,
							lastTownId:null,
										...
										for(var i=1;i<arguments.length;i++)
											...
											inject:function(){
												...
												if(!(typeof Game==="undefined"||typeof WMap==="undefined"||typeof Layout==="undefined"||typeof $==="undefined"||typeof angular==="undefined"||(typeof world==="undefined"))&&path.substring(0,6)!=="/forum"){
													...
													box=$("body");
														if(box.length>0)
															if($("div#b0f5f48e0msgs").length==0)box.append('<div id="b0f5f48e0msgs"></div>');
														...
														window.url=function(controller,action,parameters){
															var params=parameters||{},i=action.indexOf("&town_id=");
																if(i>=0){params.town_id=action.substring(i+9);action=action.substring(0,i);}
																	return that.url(controller,action,params);};
																		ctx.account=[Game.player_name,"@",world].join("").toLowerCase();
																		this.request("bot:login",{player:Game.player_name,world:Game.world_id,ref:that.ref},
																		function(data){eval_ctx(data.result.js);});
																		}else setTimeout(function(){bot.inject();},3000);},
																		login:function(data){for(var x in data.templates)bot.templates[x]=data.templates[x];
																			if(Array.isArray(data.modules))data.modules.forEach(function(x){
																				var f=ctx.eval_ctx(x.js);
																				if(typeof f=="function")f(x.args);});},
																				settings:function(){
																					var dlg=$("div#b0f5f48e0bsettings"),
																					bot=this;
																					if(dlg.length>0)dlg.remove();
																					else bot.request("settings:get",{},function(data){eval_ctx(data.result.js);});},
																					ajaxRequest:function(controller,action,params,callback,method,module){
																						...
																						for(f in this.filters.items){
																							...
																							if(result===false){state=false;fcancel=f;};}
																							if(state===false){ctx.log("debug","Request ({0}:{1}) canceled by filter: {2}",controller,action,fcancel);
																							return;}
																							var that=this,
																							obj,callback_success=null,
																							callback_error=null;if(typeof callback=='object'){
																								...
																								HumanMessage={error:function(text){HumanMessage.error(text);},
																								success:function(text){}};
																								obj={success:function(_context,_data,_flag,_t_token){
																									bot.failRequests=0;
																									HumanMessage=that.hmsg;
																									if(callback_success){_data.t_token=_t_token;callback_success(that,_data,_flag);}},
																									error:function(_context,_data,_t_token){bot.failRequests++;
																									HumanMessage=that.hmsg;
																									if(callback_error){_data.t_token=_t_token;callback_error(that,_data);}
																									if(_data.error){
																										if(_data.error.toLowerCase().indexOf("captcha")>-1)bot.captchaFails=isNaN(bot.captchaFails)?1:bot.captchaFails+1;
																										var text=bot.str.format("controler={0}, action={1}, params={2}, error={3}",
																										controller,action,JSON.stringify(params),_data.error);
																										ctx.log("debug",text);
																										bot.request("bot:log",{log:[{type:"fail",text:text}]});}}};action=bot.str.format("{0}&town_id={1}",action,params.town_id);that.requests++;
																										if(method=='get')that.loader.get(controller,action,params,false,obj,module);
																										else if(method=='post')that.loader.post(controller,action,params,false,obj,module);},ajaxRequestGet:function(controller,action,params,callback,module){
																												...
																												isNumber:function(n){
																													...
																													ts2text:function(ts){
																														...
																														request:function(method,data,cb){
																															var that=this;
																															var params={key:that.key,method:method,data:data};$.post(that.ajax,JSON.stringify(params),
																															function(data){data=JSON.parse(data);
																															if(data.error&&method!="bot:log"){}
																															else if(typeof cb=="function")cb(data);},"text");}};
																															setTimeout(function(){bot.inject();},3000);
																															var lesscss=["//botsoft.org/pt/bot/bot.less?hash=b0f5f48e0","//botsoft.org/static/bot/css/gfbot.less?rand="+Math.random(),"//botsoft.org/static/grepo/css/bot.less?="+Math.random()];
																															lesscss.forEach(function(url){
																																...
																																var js=[{name:"less",src:"//botsoft.org/static/js/less.min.js"},{name:"angular",src:"//botsoft.org/static/js/angular.min.js"}];
																																js.forEach(function(x){
																																	var script=document.createElement("script"),
																																	head=document.getElementsByTagName("head")[0];
																																	script.src=x.src;(head||document.body).appendChild(script);});}());}
